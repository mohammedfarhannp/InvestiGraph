    ...
    loop {
        ...
        for i in start_x..end_x {
            for j in start_y..end_y {
                ...
                if screen_x >= -GRID_SPACING && screen_x <= SCREEN_WIDTH + GRID_SPACING && screen_y >= RIBBON_HEIGHT {
        ...
        if is_mouse_button_down(MouseButton::Left) {
            if !camera_dragging {
                ...
                camera.x = drag_start_camera.0 + (mouse_x - drag_start.0);
                camera.y = drag_start_camera.1 + (mouse_y - drag_start.1);
            }
        } else {
